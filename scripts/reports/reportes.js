(function () {
  ...
  function formatManualFullName(nombre, apellido, fallback = '—') {
    const first = nombre ? String(nombre).trim() : '';
    const last = apellido ? String(apellido).trim() : '';
  ...
  function toManualTitleCase(value) {
    if (!value) {
      return '—';
    }
  ...
  function formatManualRequestState(state) {
    if (!state) {
      return '—';
    }
  ...
  function formatManualUserReference(id) {
  ...
  function formatManualComment(value) {
    if (!value && value !== 0) {
      return '—';
    }
  ...
  async function fetchRequestHistoryReportDataset({ empresaId, exporter }) {
    const params = new URLSearchParams({ estado: 'concluidas' });
    if (empresaId) {
    ...
    if (!response.ok || !payload || payload.success === false) {
      const message = (payload && payload.message) || 'No se pudo obtener el historial de solicitudes.';
      throw new Error(message);
    }
    ...
    const rows = items.map((item) => {
      const resumen = item && item.resumen ? String(item.resumen).trim() : '—';
      const descripcion = item && item.descripcion ? String(item.descripcion).trim() : '—';
      const modulo = toManualTitleCase(item && item.modulo);
      const accion = toManualTitleCase(item && item.tipo_accion);
      const solicitante = formatManualFullName(
        item && item.solicitante_nombre,
        item && item.solicitante_apellido,
        formatManualUserReference(item && item.id_solicitante)
      );
      const revisor = formatManualFullName(
        item && item.revisor_nombre,
        item && item.revisor_apellido,
        formatManualUserReference(item && item.id_revisor)
      );
      const estado = formatManualRequestState(item && item.estado);
      const fechaSolicitud = formatManualDateTime(item && (item.fecha_creacion || item.created_at));
      const fechaResolucion = formatManualDateTime(item && item.fecha_resolucion);
      const comentario = formatManualComment(item && item.comentario);
    ...
    if (exporter && typeof exporter.pluralize === 'function') {
      subtitleParts.push(exporter.pluralize(rows.length, 'solicitud resuelta'));
    } else {
      subtitleParts.push(`${rows.length} solicitud${rows.length === 1 ? '' : 'es'} resuelta${rows.length === 1 ? '' : 's'}`);
    }

    const aprobadas = items.filter((item) => String(item && item.estado).toLowerCase() === 'aceptada').length;
    const rechazadas = items.filter((item) => String(item && item.estado).toLowerCase() === 'denegada').length;
    if (aprobadas > 0) {
      subtitleParts.push(`Aceptadas: ${aprobadas}`);
    }
    if (rechazadas > 0) {
      subtitleParts.push(`Denegadas: ${rechazadas}`);
    }
    ...
    return {
      ...
      subtitle: subtitleParts.join(' • '),
      fileNameBase: 'historial_solicitudes',
      sheetName: 'Solicitudes',
      historyLabel: 'Historial de solicitudes',
      notes: {
        pdf: 'Exportación del historial de solicitudes resueltas (generada desde la página de reportes).',
        excel: 'Exportación del historial de solicitudes resueltas (generada desde la página de reportes).'
      },
      source: 'Log de control'
    };
  }
  ...
  const MANUAL_REPORTS = [
    ...
    },
    {
      id: 'requests-history',
      title: 'Historial de solicitudes',
      description: 'Consulta las solicitudes resueltas del log de actividades junto con sus revisiones.',
      formats: ['pdf', 'excel'],
      fetchDataset: fetchRequestHistoryReportDataset,
      historySource: 'Log de control'
    }
  ];
